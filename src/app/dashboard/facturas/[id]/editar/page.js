// Este archivo es para editar una factura existente
'use client'
...
// Componente principal para editar una factura
export default function EditarFactura({ params }) {
  ...
  // Cuando se monta el componente, pido la factura, los clientes y productos a la API
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Pido la factura, los clientes y productos al mismo tiempo
        const [facturaResponse, clientesResponse, productosResponse] = await Promise.all([
        ...
        // Verifico que la factura esté en estado borrador
        if (facturaData.estado !== 'borrador') {
          setError('Solo se pueden editar facturas en estado borrador')
          setLoading(false)
          return
        }
        ...
        // Preparo los datos de la factura para el formulario
        const formattedData = {
          ...facturaData,
          cliente: facturaData.cliente?._id || facturaData.cliente,
          fecha: facturaData.fecha ? new Date(facturaData.fecha).toISOString().split('T')[0] : '',
          fechaVencimiento: facturaData.fechaVencimiento ? new Date(facturaData.fechaVencimiento).toISOString().split('T')[0] : '',
          lineas: facturaData.lineas ? facturaData.lineas.map(linea => ({
            ...linea,
            producto: linea.producto?._id || linea.producto,
            cantidad: linea.cantidad || 0,
            precio: linea.precio || 0,
            impuesto: linea.impuesto || 0
          })) : []
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Si se actualiza bien, voy a la página de detalle de la factura
  ...
  // Si la factura no está en estado borrador, muestro un aviso
  if (error && error === 'Solo se pueden editar facturas en estado borrador') {
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold">Editar Factura</h1>
          <Link
            href={`/dashboard/facturas/${id}`}
        ...
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-3 rounded relative" role="alert">
          ...
          <p className="mt-2">Solo se pueden editar facturas que estén en estado borrador.</p>
        </div>
      </div>
    )
  }
                    ...
                    {formData.lineas.map((linea, index) => {
                      ...
                      return (
                        <tr key={index} className="border-t">
                          ...
                          <td className="px-4 py-2">{Number(linea.precio).toFixed(2)} €</td>
                    ...
                    <div key={index} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-gray-900">{Number(linea.precio).toFixed(2)} €</p>
                        </div>
                        <div>
